...
def transDH(dh):
    ...
    T = ([[np.cos(dh[0]),  -np.sin(dh[0])*np.cos(dh[3]),  np.sin(dh[0])*np.sin(dh[3]),   dh[3]*np.cos(dh[0])],
            [np.sin(dh[0]),  np.cos(dh[0])*np.cos(dh[3]),   -np.cos(dh[0])*np.sin(dh[3]),  dh[3]*np.sin(dh[0])],
            [0,           np.sin(dh[3]),              np.cos(dh[3]),              dh[2]           ],
            [0,           0,                      0,                       1]])
    return T
